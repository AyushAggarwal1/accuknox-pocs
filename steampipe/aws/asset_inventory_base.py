...
class AssetInventoryBase(BaseModule):
    ...
    selection_string_dict = {
        ...
        "aws_account": {"columns": "account_aliases, arn, title, akas, partition, region, account_id"},
        "aws_lambda_function": {
            "columns": "name, arn, code_sha_256, code_size, dead_letter_config_target_arn, description, handler, kms_key_arn, last_modified, timeout, version, package_type, master_arn, memory_size, revision_id, role, runtime, state, state_reason, state_reason_code, last_update_status, last_update_status_reason, last_update_status_reason_code, reserved_concurrent_executions, vpc_id, architectures, code, environment_variables, file_system_configs, ephemeral_storage, policy, policy_std, tracing_config, snap_start, logging_config, vpc_security_group_ids, vpc_subnet_ids, title, tags, akas, layers, partition, region, account_id, sp_connection_name, sp_ctx, _ctx",
        },
        "azure_sql_database": {
            "columns": "name, id, server_name, status, type, containment_state, creation_date, current_service_objective_id, database_id, default_secondary_location, earliest_restore_date, edition, elastic_pool_name, failover_group_id, kind, location, max_size_bytes, recovery_services_recovery_point_resource_id, requested_service_objective_id, restore_point_in_time, requested_service_objective_name, source_database_deletion_date, source_database_id, zone_redundant, create_mode, read_scale, recommended_index, sample_name, service_level_objective, service_tier_advisors, title, tags, akas, region, resource_group, cloud_environment, subscription_id, sp_connection_name, sp_ctx, _ctx",
        },
    }
    ...
    def construct_oci_steampipe_select_query(self, table):
    ...
    def _get_data(self, table):
        ...
        as a parameter. It paginates the results 100 at a time to handle large datasets.
        """
        ...
        if self._module_name == "aws_asset_inventory":
            ...
            region_info = (
                f"oci_region: {os.environ.get('OCI_REGION', getattr(self, 'region', 'no region specified for oci'))}"
            )
        if self._module_name == "oci_asset_inventory":
            steampipe_select_query = self.construct_oci_steampipe_select_query(table)
        else:
        ...
        while True:
            ...
            paginated_query = f"{steampipe_select_query} LIMIT {page_size} OFFSET {offset}"
            query = f'''steampipe query "{steampipe_select_query}" --output json'''
            # query = f'''su steampipeuser -m -c "steampipe query \\"{paginated_query}\\" --output json"'''
            ...
            query_time = time.time() - current_time
            total_time += query_time

            print(f"It took {query_time} seconds to fetch page at offset {offset} for {table}")
            ...
            try:
                page_result = json.loads(process.stdout.decode("utf-8", "ignore"))
            ...
            # For the first page, keep the full output structure
            if all_results is None:
                all_results = page_result
            else:
                # For subsequent pages, extract rows and append to the first output
                if "rows" in page_result and page_result["rows"]:
                    all_results["rows"].extend(page_result["rows"])

            # Check if we got fewer results than page_size, indicating last page
            if "rows" not in page_result or len(page_result["rows"]) < page_size:
                break

            offset += page_size

        print(f"Total time to fetch all data for {table}: {total_time} seconds")
        if all_results and "rows" in all_results:
            print(f"Total rows fetched for {table}: {len(all_results['rows'])}")
